...
# Update the Kontell repository with the latest pvr.kofin release from GitHub.
#
# Usage:
#   ./scripts/update-repo.sh [zips-directory]
#
...
if [[ $# -ge 1 ]]; then
    ...
    gh release download "$release_tag" --repo "$ADDON_REPO" --pattern "pvr.kofin-*.zip" --dir "$ZIPS_DIR"
    echo ""
fi
...
platform_dir() {
    local os="$1" arch="$2"
    echo "pvr.kofin+${os}-${arch}"
}

echo "Updating Kontell repository from $ZIPS_DIR"
echo ""

# Find all pvr.kofin zips and process them
found=0
for zip in "$ZIPS_DIR"/pvr.kofin-*-kodi*.zip; do
    [[ -f "$zip" ]] || continue
    filename="$(basename "$zip")"

    # Parse: pvr.kofin-<ver>-<os>-<arch>-kodi<N>.zip
    if [[ "$filename" =~ ^pvr\.kofin-([^-]+)-([^-]+)-([^-]+)-kodi([0-9]+)\.zip$ ]]; then
        ver="${BASH_REMATCH[1]}"
        os="${BASH_REMATCH[2]}"
        arch="${BASH_REMATCH[3]}"
        kodi="${BASH_REMATCH[4]}"
    ...
    dest_dir="$PAGES_DIR/$version_dir/$plat_dir"
    dest_file="$dest_dir/pvr.kofin-${ver}.zip"
...
if [[ $found -eq 0 ]]; then
    echo "Error: no pvr.kofin zips found in $ZIPS_DIR"
    exit 1
fi
...
echo "  ./scripts/publish.sh \"update pvr.kofin\""
