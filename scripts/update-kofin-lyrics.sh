...
# Update the Kontell repository with the latest script.kofin.lyrics zip.
#
# Usage:
#   ./scripts/update-kofin-lyrics.sh [zip-path]
#
...
#
# script.kofin.lyrics is pure Python (<platform>all</platform>), so the same zip
# is used for both Kodi 21 (omega) and Kodi 22 (piers) — no per-platform matrix.
#
# plugin.video.kofin declares this addon as a non-optional dependency, so it has
# to be in the repository before a kofin release that requires it: Kodi refuses
# to install an addon whose imports it cannot resolve. Run this one first.
#
...
ADDON_ID="script.kofin.lyrics"
ADDON_REPO="kontell/script.kofin.lyrics"
...
if [[ $# -ge 1 ]]; then
    ...
    if [[ -f "$arg" ]]; then
        ...
        # Pick the latest lyrics zip in the directory
...
# Parse: script.kofin.lyrics-<ver>.zip
...
echo "Adding Kofin Lyrics $ver to Kontell repository"
echo ""
...
echo "  ./scripts/publish.sh \"update script.kofin.lyrics\""
